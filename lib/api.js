...
class Api {
	...
	async massEdit(list, mode, summary) {
		if (mode !== 'dry') {
			...
			for (const [pageid,, text] of list) {
				try {
					/* eslint-disable no-await-in-loop */
					if (text === null) {
						await this.edit({pageid, appendtext: '', summary: '空编辑'});
					} else {
						await this.edit({pageid, text, summary: `${summary}，如有错误请联系[[User talk:Bhsd|Bhsd]]`});
	...
	async #revisions(params) {
		...
		const pages = query.pages.filter(({revisions}) => revisions && revisions[0].contentmodel === 'wikitext')
			.map(({pageid, title, revisions: [{content}]}) => ({pageid, title, content}))
