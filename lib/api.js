...
const Rp = require('./request-promise.js'),
	{error, isObject, sleep, save, diff, info} = require('./dev.js'),
...
class Api {
	...
	async edit(params) {
		...
		const form = {
			action: 'edit', nocreate: 1, minor: 1, summary: '测试性编辑', token: this.#token, ...params,
	...
	async massEdit(list, mode, summary) {
		if (mode !== 'dry') {
			...
			for (const [pageid,, text, basetimestamp, starttimestamp] of list) {
				...
				try {
					...
					if (text === null) {
						await this.edit({pageid, appendtext: '', summary: '空编辑'});
					} else {
						await this.edit({
							pageid, text, basetimestamp, starttimestamp,
							summary: `${summary}，如有错误请联系[[User talk:Bhsd|用户Bhsd]]`,
						});
		...
		save('../../Bhsd-bot-public/config/dry.json', list.map(([pageid,, ...rest]) => [pageid, null, ...rest]));
		(await Promise.all(list.map(async ([pageid, content, text], i) => {
			...
			fs.unlink(`oldcontent${i}`);
			fs.unlink(`newcontent${i}`);
			return [pageid, diffOut];
		}))).filter(page => page?.[1]).forEach(([pageid, diffOut]) => {
			info(`${pageid}:`);
			console.log(diffOut);
		});
	}
	...
	async #revisions(params) {
		...
		const pages = query.pages.filter(({revisions}) => revisions && revisions[0].contentmodel === 'wikitext')
			.map(({pageid, title, revisions: [{content, timestamp}]}) => ({
				pageid, title, content, timestamp, curtimestamp,
	...
	categorymembers(gcmtitle, params = {}) {
