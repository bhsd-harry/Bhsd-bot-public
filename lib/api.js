...
class Api {
	...
	async login() {
		...
		const lgtoken = (await this.get({meta: 'tokens', type: 'login'})).query.tokens.logintoken;
	...
	async csrfToken() {
		...
		let {csrftoken} = (await this.get({meta: 'tokens'})).query.tokens;
		if (csrftoken === '+\\') {
			this.logout();
			await this.login();
			({csrftoken} = (await this.get({meta: 'tokens'})).query.tokens);
		}
	...
	async edit(params) {
		...
		try {
			const {edit} = await this.post(form);
	...
	// 批量编辑，此函数不应手动执行
	async massEdit(list, mode, summary, ids = []) {
		if (mode !== 'dry') {
			...
			} else if (mode === 'rerun' && ids.length > 0) {
				list = list.filter(([pageid]) => ids.includes(String(pageid)));
			}
			if (mode !== 'redry') {
				const newtimestamps = {};
	...
	async #revisions({inuse, ...params}) {
		...
		const pages = query.pages.filter(
			...
			const [{content, timestamp}] = page.revisions;
	...
	async parse(params) {
		...
		const qs = {action: 'parse', prop: 'wikitext|parsewarnings', ...params},
			{wikitext, parsewarnings} = (await this.get(qs)).parse;
	...
	async embeddedIn(ei) {
		...
		const {embeddedin} = (await this.get(qs)).query;
		return embeddedin;
	}
}
