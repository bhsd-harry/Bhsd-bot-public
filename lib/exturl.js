...
const exturl = async (pages, chat) => {
	...
	const edits = (await Promise.all(pages.map(async ({pageid, content, oldContent, urls, timestamp, curtimestamp}) => {
		let text = content.replace(
			/(?<=\/\/)\W*t\W*i\W*e\W*b\W*a[^\w.]*\.\W*b\W*a\W*i\W*d\W*u[^\w.]*\.\W*c\W*o\W*m\//gi,
			'tieba.baidu.com/',
		).replace(/(?<=\/\/)\W*p\W*a\W*n[^\w.]*\.\W*b\W*a\W*i\W*d\W*u[^\w.]*\.\W*c\W*o\W*m\//gi, '');
		oldContent ||= content;
...
const broken = async ({content, pageid, timestamp, curtimestamp}, chat, force, incomplete) => {
	...
	const parsed = typeof content === 'string' ? Parser.parse(content, false, 9) : content,
	...
	for (const token of parsed.querySelectorAll('template#Template\\:Cite_web')) {
	...
	let nBroken = 0,
		nArchived = 0,
		nFailed = 0;
	const tokens = parsed.querySelectorAll('free-ext-link, ext-link-url, template#Template\\:Cite_web').filter(token => {
	...
	await Promise.all(tokens.slice(0, 20).map(async token => {
		if (token.type === 'template' && token.getValue('dead-url') !== 'yes') {
			token.setValue('dead-url', 'yes');
		}
		const link = (token.type === 'template' ? token.getValue('url') : token.text()).replace(/^\/\//, 'https://'),
			[archiveUrl, date, template] = await archive(link, chat);
		if (!template && !token.dead) {
			...
			if (token.langMark && token.type === 'template') {
