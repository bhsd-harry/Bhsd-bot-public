...
const broken = async ({content, pageid, timestamp, curtimestamp}, chat, force) => {
	...
	for (const token of parsed.querySelectorAll('ext-link-url, free-ext-link')) {
		const {previousElementSibling, nextElementSibling} = token.type === 'free-ext-link'
			? token
			: token.parentNode;
		token.dead = nextElementSibling?.matches(selectors) && nextElementSibling;
...
const archive = async (link, chat, retry) => {
	...
	if (chat) {
		...
		title = await chat.push(htmlTitle, {404: ['请输入正确的存档网址：']});
