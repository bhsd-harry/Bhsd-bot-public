...
const broken = async ({content, pageid, timestamp, curtimestamp}, chat, force, incomplete) => {
	...
	for (const token of parsed.querySelectorAll('template#Template\\:Cite_web, template#Template\\:Cite_news')) {
	...
	let nBroken = 0,
		...
		nFailed = 0;
	const tokens = parsed
		.querySelectorAll('free-ext-link, ext-link-url, template#Template\\:Cite_web, template#Template\\:Cite_news')
		.filter(token => {
