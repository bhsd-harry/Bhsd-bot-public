...
const broken = async ({content, pageid, timestamp, curtimestamp}, chat, force) => {
	...
	if (!force && new Date(timestamp).getTime() <= new Date(archived[pageid]).getTime()) { // 无该页面记录时总是false
	...
	let nBroken = 0,
		nArchived = 0,
		nFailed = 0;
	await Promise.all(parsed.querySelectorAll('free-ext-link, ext-link-url, template#Template\\:Cite_web')
		.map(async token => {
