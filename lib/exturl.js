...
const _format = url => url.replace(/:\d+$/u, '').split('.').reverse().join('.');
...
const broken = async ({content, pageid, timestamp, curtimestamp}, chat, force, incomplete) => {
	...
	const /** @type {Parser.Token} */ parsed = typeof content === 'string' ? Parser.parse(content, false, 9) : content,
	...
	let urls = [],
		deadUrls = [];
	const /** @type {Parser.MagicLinkToken[]} */ magicLinks = parsed.querySelectorAll('ext-link-url, free-ext-link');
	for (const token of magicLinks) {
		...
		if (!token.dead?.hasArg('bot')) {
			(token.dead ? deadUrls : urls).push(token.text().replace(/^\/\//u, 'https://'));
		}
	}
	const templates = parsed.querySelectorAll(String.raw`template#Template\:Cite_web, template#Template\:Cite_news`);
	for (const token of templates) {
		...
		if (!token.dead?.hasArg('bot')) {
			(token.getValue('dead-url') === 'yes' ? deadUrls : urls)
				.push(token.getValue('url').replace(/^\/\//u, 'https://'));
		}
	}
	...
	const tokens = parsed
		.querySelectorAll(
			String.raw`free-ext-link, ext-link-url, template#Template\:Cite_web, template#Template\:Cite_news`,
		)
		.filter(token => {
			if (token.type === 'free-ext-link' && token.isParamValue() || token.type === 'template' && !token.main) {
				return false;
			}
			const link = (token.type === 'template' ? token.getValue('url') : token.text()).replace(/^\/\//u, 'https://');
	...
	await Promise.all(tokens.slice(0, 20).map(async token => {
		...
		const link = (isTemplate ? token.getValue('url') : token.text()).replace(/^\/\//u, 'https://'),
			[archiveUrl, date, template] = await archive(link, chat, isTemplate);
		if (!template && !token.dead) {
			...
			if (token.langMark && isTemplate) {
				token.setValue('language', token.langMark.name.slice(9).toLowerCase());
			}
			if (isTemplate) {
