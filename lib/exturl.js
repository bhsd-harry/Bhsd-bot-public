 ...
 */
const broken = async ({content, pageid, timestamp, curtimestamp}, chat, force, api) => {
	let archived = {},
		noChange = false;
	try {
	...
	for (const token of bilis) {
		const link = `${
				token.name.startsWith('Template:Bili') ? 'https://www.bilibili.com/video' : 'https://www.acfun.cn/v'
			}/${token.getValue(1)}`,
