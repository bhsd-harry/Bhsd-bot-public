...
const broken = async (content, pageid) => {
	...
	await parsed.each('url', async (token, _, parent) => {
		...
		if (responses.includes(link)) {
			const template = await archive(link);
...
const archive = async link => {
	...
	const [url, date, title, lang] = response;
	return `{{Cite web|url=${link}|${lang && !lang.startsWith('zh')
		? `language=${lang}|script-title=${lang}:`
		: 'title='
	}${title.replaceAll('|', '{{!}}')}|archive-url=${url}|archive-date=${date}|dead-url=yes}}`;
};
