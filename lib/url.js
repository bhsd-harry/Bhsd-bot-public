'use strict';
const {urlRegex, error, isObject} = require('./dev'),
	Parser = require('wikiparser-node');
Parser.config = './config/moegirl';

/** @type {RegExp} 其他PHP script（仅用于wgScriptPath和wgArticlePath一致的情形） */
const scripts = /^(?:api|load|thumb|img_auth|rest)\.php(?:[/?]|$)/u,
	...
	langs = /^\/zh-(?:han[st]|cn|tw|hk)\//u,
	/** @type {string[]} 不可改写为内链的常见action参数 */
	actions = ['history', 'info', 'watch', 'unwatch', 'rollback', 'render', 'submit', 'edit', 'raw'],
	/** @type {RegExp} 必须以':'开头的内链 */
	nsEscape = /^(?:file|[图圖]像|[档檔]案|文件|image|cat(?:egory)?|分[类類]):/iu,
	/** @type {RegExp} 忽略用户页和用户讨论页 */
	nsUser = /^(?:user(?:[ _]talk)?|u|用[户戶](?:对话|對話|讨论|討論)?|使用者(?:討論)?):/iu;

const unknownKey = (searchParams, knownKeys) => [...searchParams.keys()].some(key => !knownKeys.includes(key));
...
class WikiUrl {
	...
	constructor(wgServer, wgScriptPath, wgArticlePath = wgScriptPath, wgVariantArticlePath = true) {
		...
		/** @type {RegExp} 外链格式 */
		this.#regex = new RegExp( // eslint-disable-line require-unicode-regexp
			String.raw`\[{1,2}((?:https?:)?//(?:${
				this.server.map(RegExp.escape).join('|')
			})/${urlRegex}+)([\s[<>"][^\]]*)?\s*\]{1,2}`,
			'gi',
		);
	}
	...
	query(anchor, pageid = 0) {
		...
		try {
			hash &&= decodeURIComponent(hash.replace(/\.(?=[A-F\d]{2})/gu, '%'))
				.replace(/[|[\]{}&<]/gu, c => encodeURI(c));
	...
	getTitle(anchor, pageid = 0) {
		...
		if (action !== 'view') { // 情形2: 不是view
			...
			if (unknownKey(searchParams, searchKeys)) { // 情形4: 有未知的URL参数
		...
		} else if (unknownKey(searchParams, keys)) { // 情形4: 有未知的URL参数
