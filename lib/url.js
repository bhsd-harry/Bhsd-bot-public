const {urlRegex, escapeRegExp, parse, error, isObject} = require('./dev');
...
class WikiUrl {
	...
	getTitle(anchor, pageid = 0) {
		...
		if (action !== 'view') { // 情形2: 不是view
			...
			if (profile !== 'all' || offset !== '0') {
				console.log(`${pageid}: 指定搜索参数`, Object.fromEntries(searchParams));
				return null;
			}
	...
	replace(wikitext, pageid) {
		...
		parsed.each(
			(type, _, parentType) => ['file', 'url'].includes(type) && parentType !== 'external_link',
			token => {
				if (token.type === 'file') {
					...
					const [link] = token;
					if (typeof link !== 'string' || token.length > 1) {
						console.error('未预期的url', token);
						return;
					}
					token[0] = this.#convert(link, link, null, pageid);
				}
			},
		);
