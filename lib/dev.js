...
const diff = async (oldfile, newfile, stdin) => {
	...
	// ], 'hex');
	if (stdin) {
		await Promise.all([promises.unlink('oldcontent'), promises.unlink('newcontent')]);
	}
	return stdout.split('\n').slice(4).join('\n');
	/*
...
const contentLength = async url => {
	...
	const length = Number(response.match(/(?<=content-length:)\s*(\d+)(?=\s)/gi)?.at(-1));
...
const wayback = async brokenUrl => {
	...
	const title = head.match(/(?<=<title[^>]*>)[^<]+/)?.[0]?.replaceAll('\n', ' ');
 ...
 * @returns {string}
 */
const trim = (str = '') => str.replace(/^[\s\u200e]+/, '').replace(/(?<![\s\u200e])[\s\u200e]+$/, '');
/**
 * 解码HTML实体
 * @param {string} str
 * @returns {string}
