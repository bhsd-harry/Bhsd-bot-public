...
const diff = async (oldfile, newfile, stdin) => {
	...
	const start = Buffer.from('\x1b[7;31m').toString('hex'),
		middle = Buffer.from('\x1b[m\x1b[7;32m').toString('hex'),
		pattern = new RegExp(`e3${start}(.{4})${middle}`, 'g');
	return stdout && Buffer.from(
		stdout.split('0a').slice(4).join('0a').replace(pattern, `${start}e3$1${middle}e3`),
		'hex',
	).toString();
};
/**
...
const wayback = async brokenUrl => {
	...
	try {
		const {url, timestamp} = await new Promise((resolve, reject) => {
		...
		if (!fs.existsSync(file)) {
			await _cmd('curl', ['-r', '0-100000', '-o', file, url]);
		}
		let head = fs.readFileSync(file, 'utf8');
		const encoding = head.match(/(?<=\sencoding=").+?(?=")/)?.[0];
		if (encoding) {
			head = await _cmd('iconv', ['-f', encoding, '-t', 'utf-8', file]);
		}
		const title = head.match(/(?<=<title.*?>).+?(?=<\/title>)/)?.[0],
			lang = head.match(/(?<=\s(?:xml:)?lang=").+?(?=")/)?.[0]?.split('-', 1)?.[0]?.toLowerCase();
		fs.unlinkSync(file);
		return [
			`https${url.slice(4)}`,
...
const parse = text => {
	...
	if (!global.CeL?.wiki?.parser) {
		require('../../CeJS/CeJS-customized/ce.js');
		CeL.run('application.net.wiki.parser');
	}
	return new CeL.wiki.parser(text).parse();
};
