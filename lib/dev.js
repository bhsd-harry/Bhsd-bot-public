...
const diff = async (oldfile, newfile, stdin) => {
	...
	const start = Buffer.from('\x1b[7;31m').toString('hex'),
		...
		pattern = new RegExp(`([c-f].(?:[8-9a-b].)?)${start}(.{4,}?)${middle}`, 'g');
	return stdout && Buffer.from(
		stdout.split('0a').slice(4).join('0a').replace(pattern, `${start}$1$2${middle}$1`),
		'hex',
	).toString();
};
/**
...
const wayback = async brokenUrl => {
	...
	try {
		...
		let lang = head.match(/(?<=\s(?:xml:)?lang=").+?(?=")/)?.[0]?.split('-', 1)?.[0]?.toLowerCase();
		if (lang === 'en') {
