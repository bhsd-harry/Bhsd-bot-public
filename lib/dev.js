...
const sleep = async t => {
	while (t >= 1) {
		refreshStdout(String(Math.floor(t)));
		await _sleep(1);
		t--;
	}
	await _sleep(t);
...
const error = (msg = 'dev.error测试', ...args) => {
	console.log(styleText('red', msg), ...args);
};
...
const info = (msg = 'dev.info测试', ...args) => {
	console.log(styleText('green', msg), ...args);
};
...
const runMode = (modes = []) => {
	...
	if (mode && !['dry', 'redry', 'run', 'rerun', ...typeof modes === 'string' ? [modes] : modes].includes(mode)) {
 ...
 * @returns {Promise<string>}
 */
const diff = async (oldcontent, newcontent, stdin) => {
	let oldfile = oldcontent,
		newfile = newcontent;
	if (stdin) {
		oldfile = `oldcontent-${createHash('sha256').update(oldcontent).digest('hex')}`;
		newfile = `newcontent-${createHash('sha256').update(newcontent).digest('hex')}`;
...
const yahoo = async (str, options = {}) => {
	...
	fs.writeFileSync('cache/lyrics', str);
	fs.writeFileSync('cache/ruby-yahoo', ruby);
	return ruby;
};
