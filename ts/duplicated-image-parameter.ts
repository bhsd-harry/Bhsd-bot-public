...
const main = async (api = new Api(user, pin, url)) => {
	...
	for (const {pageid, content, timestamp, curtimestamp} of pages) {
		...
		for (const parameter of root.querySelectorAll<Parser.ImageParameterToken>(selector)) {
			if (!root.contains(parameter)) {
				continue;
			}
			const {parentNode: {childNodes, type}, name: curName, value: curValue} = parameter as Parser.ImageParameterToken & {value: string},
				i = childNodes.indexOf(parameter),
				repeated = childNodes.slice(i + 1).filter(({name}) => name === curName) as (Parser.ImageParameterToken & {value: string})[];
			if (curName === 'invalid') {
				parameter.remove();
				continue;
				...
				const [{value}] = repeated as (Parser.ImageParameterToken & {value: string})[];
