...
class WikiUrl {
	...
	query(anchor, pageid = 0) {
		...
		try {
			hash &&= decodeURIComponent(hash.replace(/\.(?=[A-F\d]{2})/g, '%'))
	...
	replace(wikitext, pageid) {
		...
		for (const fileLink of parsed.querySelectorAll<Parser.ImageParameterToken>('image-parameter#link')) {
			const link = (fileLink.getValue() as string).trim();
		...
		for (const freeExt of parsed.querySelectorAll<Parser.MagicLinkToken>('free-ext-link')) {
