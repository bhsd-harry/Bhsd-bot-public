...
const _analyze = (wikitext, pageid, ns) => {
	...
	const comments = root.querySelectorAll<Parser.CommentToken>('[duplication]:is(template, magic-word#invoke) comment[closed=true]')
	...
	for (const comment of comments) {
		...
		if (previousSibling?.data?.endsWith(' • ')) {
			(previousSibling as Parser.AstText).deleteData(-3, 3);
		}
	}
		...
		if (keys.size === 0) {
			...
			for (const key of keys) {
				if (/^in(?:\d+年)?(?:\d+月)?(?:\d+日)?$/.test(key)) {
					let i = 2;
					const attempt = (arg: Parser.ParameterToken) => {
						try {
							arg.rename(`${key}#${i}`);
						} catch {
							i++;
